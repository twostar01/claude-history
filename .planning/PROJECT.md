...
**Shipped:** v1.1 (2026-05-18) — search date/role filters, file export, incremental ingest.

## Current Milestone: v1.2 (not yet defined)

See `/gsd-new-milestone` to start the next milestone.
...
### Validated (v1.1 — shipped 2026-05-18)

- ✓ `search_conversations` supports `date_from` / `date_to` filter parameters — v1.1 (Phase 5, SRCH-01)
- ✓ `search_conversations` supports `role_filter` parameter — v1.1 (Phase 5, SRCH-02)
- ✓ `export_conversation` can optionally write markdown to a file path — v1.1 (Phase 5, EXP-01)
- ✓ Incremental ingest indexes new messages in existing conversations — v1.1 (Phase 6, INGEST-01)

### Deferred

- [ ] Ingest script indexes PDF attachment content (ATTACH-01) — deferred from v1.1; requires pypdf/pdfplumber; narrow value for personal use
...
**Current state:** v1.1 shipped (2026-05-18). Search filters (date/role), file export, and incremental ingest all live. Server registered in user scope (`~/.claude.json`). 106 conversations (4087 messages, 9.88 MB DB) indexed from real Claude.ai export.
...
---
*Last updated: 2026-05-20 — v1.1 milestone archived*
